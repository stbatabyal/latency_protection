...
histPlot0[data_, bins_: 10, opt___] := 
 Module[{countBorder = 
...
(* 
histPlot[data_, bins_: 10, type_: "Probability", opt___] :=
 Module[{countBorder =
    Partition[Riffle[Riffle[#1, #1[[2 ;;]]], Riffle[#2, #2]], 2] & @@
     HistogramList[data, bins, type]},
  ListLinePlot[countBorder, opt, Options[MyPlot]]
]
*)


histPlot[dataDD_, bins_ : 10, type_ : "Probability", opt___] := 
 Module[{dataNN, countBorder, ii},
  If[Length[Dimensions[data]] == 1,
   countBorder = 
    Partition[Riffle[Riffle[#1, #1[[2 ;;]]], Riffle[#2, #2]], 2] & @@ 
     HistogramList[dataDD, bins, type];
   ListLinePlot[countBorder, opt, 
    PlotStyle -> {{Thickness[0.006], dashings[[1]], colors[[1]]}}, 
    Options[MyPlot]],
   Show[
    Table[
     dataNN = dataDD[[ii]];
     countBorder = 
      Partition[Riffle[Riffle[#1, #1[[2 ;;]]], Riffle[#2, #2]], 2] & @@
        HistogramList[dataNN, bins, type];
     ListLinePlot[countBorder, opt, 
      PlotStyle -> {{Thickness[0.006], dashings[[ii]], colors[[ii]]}},
       Options[MyPlot]],
     {ii, 1, Length[dataDD]}]
    ]
   ]
...
(*
histPlotLog[data_, bins_: 10, type_: "Probability", opt___] :=
 Module[{countBorder =
    Partition[Riffle[Riffle[#1, #1[[2 ;;]]], Riffle[#2, #2]], 2] & @@
     HistogramList[data, bins, type]},
  ListLinePlot[ToLogOne[countBorder], opt, Options[MyPlot]]
]
*)


histPlotLog[dataDD_, bins_ : 10, type_ : "Probability", opt___] := 
 Module[{dataNN, countBorder, ii},
  If[Length[Dimensions[data]] == 1,
   countBorder = 
    Partition[Riffle[Riffle[#1, #1[[2 ;;]]], Riffle[#2, #2]], 2] & @@ 
     HistogramList[dataDD, bins, type];
   ListLinePlot[ToLogOne[countBorder], opt, 
    PlotStyle -> {{Thickness[0.006], dashings[[1]], colors[[1]]}}, 
    Options[MyPlot]],
   Show[
    Table[
...
StyleBox[\"-\",\nFontSize->36]\)",{nplot}]];
     If[(PlotMarkers /. opt) == Line, myshape = Table[Graphics[{Thickness[0.15], Line[{{-0.1, 0}, {0.1, 0}}]}],{nplot}]];
...
mylegend3[tlabels_, pars__,opt___] := Module[
  ...
  Flatten[{
   ...
   Table[Text[Style[allsymbols[[j]], allcolors[[j]], Bold,3+allsizes[[j]]],
...
(* allows to include your own labels, based on MyGraphicsArray *)


MyGraphicsArray1[list_, myletters_ : CharacterRange["A", "Y"], 
   opt___] := 
  Module[{n, m, i, j, letters, dim, panels, opt1, missing}, 
   dim = Dimensions[list];
   {n, m} = If[Length[dim] == 1, {1, dim[[1]]}, dim];
   missing = Map[Map[If[StringQ[#] == True, 1, 0] &, #] &, list];
   letters = If[Length[myletters] == 0,
     Take[let0 = CharacterRange["A", "Y"];
      Join[let0, 
       Flatten[Table[
     ...
     Table[Panel[list[[i, j]], 
       If[missing[[i, j]] == 0, 
        Style[letters[[j + (i - 1)*m]], Bold, 21, 
         FontFamily -> "Arial"], Style[""]], {{Left, Top}}, 
       Background -> GrayLevel[1]], {i, 1, n}, {j, 1, m}]];
...
MyGraphicsArray0[list0_, opts___] := 
 Module[{let0, xdim, nmax, list, n, m, nf, letters, i, j, ii, opt}, 
  xdim = (Length[#1] &) /@ list0;
  opt = Flatten[Join[{opts}, Options[MyListPlot]], 1];
  nmax = Max[xdim]; 
  list = (If[Length[#1] < nmax, 
       Append[#1, Table["NA", {i, 1, nmax - Length[#1]}]], #1] &) /@ 
    list0; {n, m} = {nmax, Length[xdim]}; nf = Total[xdim];
  If[Length[Letters /. opt] > 0, let0 = Letters /. opt, 
    ...
    Table[ii++; 
     Panel[list[[i, j]], 
      Style[letters[[ii]], Bold, 21, 
       FontFamily -> "Arial"], {{Left, Top}}, 
      Background -> GrayLevel[1]], {j, 1, xdim[[i]]}], {i, 1, m}]; 
  Grid[panels, Alignment -> {Left, Top}, Spacings -> {0, 0.01`}, 
   ItemSize -> All]
  ];
